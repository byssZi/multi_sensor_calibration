...
bool genPcdFeature(pcl::PointCloud<LidarPointXYZIRT>::Ptr laserCloud,
                   ...
                   int Lidar_type_,int LIDAR_LASER_NUM_, int MAX_POINT_CLOUD_NUM_, int SCAN_LINE_CUT_, int INTENSITY_THRESHOLD_) {
  int cloud_size = laserCloud->points.size();
  

  cloudCurvature.resize(MAX_POINT_CLOUD_NUM, 0); 
  cloudSortInd.resize(MAX_POINT_CLOUD_NUM, 0);
  cloudNeighborPicked.resize(MAX_POINT_CLOUD_NUM, 0);
  cloudLabel.resize(MAX_POINT_CLOUD_NUM, 0);


  std::cout << ".............................." << std::endl;
  std::cout << "Point Size: " << cloud_size << std::endl;
  std::cout << "Max_Point_Cloud_Num: " << MAX_POINT_CLOUD_NUM << std::endl;
  std::cout << ".............................." << std::endl; 
  ...
  for (int i = 0; i < LIDAR_LASER_NUM; i++) {
    ...
    for (int j = 0; j < cur_point_num; j++) {
      ...
      if (j >= 5 && j < cur_point_num - 5) {
        ...
        {
          cloudCurvature[real_pidx] = laserCloud->points[laser_index[i][j]].timestamp / float(1000000);
        }
