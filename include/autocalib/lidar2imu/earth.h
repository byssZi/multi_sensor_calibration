...
public:
    ...
    static Matrix3d global2local_R(const Vector3d &origin, const Matrix3d &origin_R, const Vector3d &BLH, const Matrix3d &R) {
        Matrix3d cn0e  = cne(origin);
        Matrix3d cn1e  = cne(BLH);
        Eigen::Matrix3d qua;
        qua = origin_R.transpose() * cn0e.transpose() * cn1e * R;
